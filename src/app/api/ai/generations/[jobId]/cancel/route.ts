...
interface GenerationCancellationRouteContext {
  params: Promise<{ jobId: string }>
}

export async function POST(
  _request: Request,
  context: GenerationCancellationRouteContext,
) {
  try {
    ...
    const generationJob = await requestAiService(
      '/v1/generations/' + encodeURIComponent(jobId) + '/cancel',
    ...
    return handleApiError(
      error,
      'Unable to cancel module generation.',
      'ai.generations.cancel',
    )
  }
}
